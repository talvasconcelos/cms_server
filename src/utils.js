...
const utils = {
  ...
  guppy: (arr, init = 3, step = 6) => {
    const Ns = [...utils.range(init, 51, step)];
    const mma = Ns.map((n, i) => utils.EMA(arr, n));

    let guppy = [];
    for (let i = 0; i < mma[0].length; i++) {
      let fastL = mma[0][i] > mma[1][i] && mma[1][i] > mma[2][i];

      let fastS = mma[0][i] < mma[1][i] && mma[1][i] < mma[2][i];
      let slowL =
        mma[3][i] > mma[4][i] &&
        mma[4][i] > mma[5][i] &&
        mma[5][i] > mma[6][i] &&
    ...
    let relVol = vol.map((cur, i, arr) => {
      if (i > period) {
        ...
        let avgVol = lookback.reduce((acc, val, index) => {
          acc += val;
          if (index === lookback.length - 1) return acc / lookback.length;
          return acc;
        });
  ...
  prepareData: data => {
    ...
    const close = data.map(v => v[4]);
    const open = data.map(v => v[1]);
    const high = data.map(v => v[2]);
    const low = data.map(v => v[3]);
    const volume = data.map(v => v[5]);
    ...
    const AE = data
      .map((_, i) => [zOpen[i], zHigh[i], zLow[i], zClose[i], zVol[i]])
      .slice(-WINDOW);
    const X = data
      .map((_, i) => [
        ema12[i],
